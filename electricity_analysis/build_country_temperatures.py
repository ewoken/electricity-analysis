...
def get_country_data_frame(da, country_code, country_geometries):
    ...
    res = da.mean(dim='lat_lng') - 273.15
    return res.to_dataframe(country_code)


def get_countries_day_temperature_df(data_path, countries, country_geometries):
    print(data_path)
    ds = load_copernicus_dataset(data_path)
    da = ds['t2m'].resample(time="1D").mean().stack(lat_lng=["latitude", "longitude"])
...
if __name__ == "__main__":
    ...
    df_2010_2014 = get_countries_day_temperature_df(
        './_data/copernicus/europe_temperature_2010_2014.nc',
        countries,
        country_geometries
    )
    df_2015_2019 = get_countries_day_temperature_df(
        './_data/copernicus/europe_temperature_2015_2019.nc',
        countries,
        country_geometries
    )

    df = pd.concat([df_2010_2014, df_2015_2019]).rename({ 'time': 'date' })
    df.to_csv('./_data/country_day_temperatures_2010_2019.csv')
